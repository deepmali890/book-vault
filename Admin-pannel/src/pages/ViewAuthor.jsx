...
const ViewAuthor = () => {
  ...
  const fatchAuthor = () => {
    axios.get(`${import.meta.env.VITE_APP_API_HOST}/api/admin-panel/author/read-author`)
      .then((response) => {
  ...
  const fetchDeletedAuthor = () => {
    axios.get(`${import.meta.env.VITE_APP_API_HOST}/api/admin-panel/author/deleted-author`)
      .then((response) => {
        console.log(response.data)
  ...
  useEffect(() => { fatchAuthor(); fetchDeletedAuthor() }, [])

  const notify = (e) => {
    // console.log(e.target.value, e.target.textContent)
    const status = e.target.textContent !== "Enabled";
    axios.put(`${import.meta.env.VITE_APP_API_HOST}/api/admin-panel/author/update-author-status/${e.target.value}`, { status })
      .then((response) => {
        console.log(response.data)
        // fatchAuthor()
  ...
  const handleDeleteAuthor = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.put(`${import.meta.env.VITE_APP_API_HOST}/api/admin-panel/author/delete-author/${id}`)
          .then((response) => {
            console.log(response.data)
            setAuthor((pre) => (
              pre.filter((auth) => auth._id !== id)
            ))
            Swal.fire({
              title: "Deleted!",
  ...
  const restoreAuthor = (id) => {
    axios.put(`${import.meta.env.VITE_APP_API_HOST}/api/admin-panel/author/restore-author/${id}`)
      .then(() => {
        fatchAuthor()
        fetchDeletedAuthor()
  ...
  const handleParmanentDaleteParentCategory = (id) => {
    axios.delete(`${import.meta.env.VITE_APP_API_HOST}/api/admin-panel/author/delete-author/${id}`)
      .then((response) => {
        console.log(response.data)
        setAuthor((pre) => (
          pre.filter((auth) => auth._id !== id)
  ...
  const handleMultiDelete = () => {
    ...
    axios.put(`${import.meta.env.VITE_APP_API_HOST}/api/admin-panel/author/multi-delete-author`, { ids: checked })
      .then((response) => {
        console.log(response.data)
        setAuthor((pre) => (
          pre.filter((auth) => !checked.includes(auth._id))
        ))
        setChecked([])
  ...
  return (
    <div className="w-full mt-10 px-10    pb-8 h-[100vh]">
      ...
      <div className=" flex justify-between items-center">
        ...
        <Link to={'/dashboard/add-author'}> <div className="w-[40px] h-[40px] bg-[#0297B2] text-white cursor-pointer rounded-full me-4 flex justify-center items-center"><FaPen /></div> </Link>
      </div>
            ...
            <tbody className='my-2 text-nowrap'>
              {
                author.map((items, index) => (
                  <tr className=" border-b-[1px] " key={index}>
                    ...
                    <td class="px-5   bg-white  text-md ">
                      <Avatar>
                        <AvatarImage src={filepath + items.thumbnail} alt="@shadcn" />
                        <AvatarFallback></AvatarFallback>
                      </Avatar>
                    </td>
